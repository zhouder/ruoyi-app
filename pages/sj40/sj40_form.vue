...
<script>
	...
	export default {
		...
		methods: {
			...
			validateForm() {
				if (!this.form.sgdw) {
					uni.showToast({ title: '请填写施工单位', icon: 'none' });
					return false;
				}
				if (!this.form.bh) {
					uni.showToast({ title: '请填写编号', icon: 'none' });
					return false;
				}
				// 验证明细数据
				for (let i = 0; i < this.sj40DetailList.length; i++) {
					...
					if (!row.hfbh) {
						uni.showToast({ title: `第${i+1}行焊缝编号不能为空`, icon: 'none' });
						return false;
					}
				}
				return true;
